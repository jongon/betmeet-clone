...
- **Sync** — Los emparejamientos de eliminatoria recién definidos aparecen ahora en **minutos** en lugar de esperar al cron de `FIXTURES` diario (`0 6 * * *`). Solo el scope `FIXTURES` resuelve los equipos de la siguiente ronda, y el proveedor puede tardar horas en poblar un cruce tras terminar los partidos que lo alimentan (caso real: el cuarto de final ARG–SUI tardó >10h porque venía de una tanda de penales conflictiva), así que el partido se mostraba **«por definir»** hasta la sincronización del día siguiente. Ahora el cron de `RESULTS` (cada 5 min) **encadena una pasada de `FIXTURES`** mientras `isKnockoutResolutionWindow()` sea verdadero —hay un partido de knockout terminado en las últimas 24h (`KNOCKOUT_RESOLVE_LOOKBACK_MS`) y otro posterior aún sin equipos—, disparándose en el route handler del cron (`/api/cron/sync`) justo antes de la revalidación de caché que ya existía, de modo que los equipos resueltos y su invalidación de caché aterrizan en el mismo tick. En fase de grupos y una vez resuelto todo el bracket no añade llamadas al proveedor; es **best-effort** (si la pasada de `FIXTURES` falla, la corrida de `RESULTS` no se cae) y **no puede entrar en bucle** (`FIXTURES` nunca marca partidos como `FINISHED`). El `FIXTURES` diario se conserva como respaldo para fase de grupos. Con tests de regresión.
...
- **Types** — Corregido un error de tipos que dejaba `tsc --noEmit` en rojo: el test de `PoolLiveNowBanner` construía literales `PoolMemberSummary` con `avatarUrl: null`, pero ese campo es `string` no-nullable (el `User.avatarUrl` del schema es `String`, nunca `null`; solo los tipos de *predicción* usan `string | null`). Ahora usan `""` («sin avatar»), consistente con el resto del test. Sin cambios de runtime.
