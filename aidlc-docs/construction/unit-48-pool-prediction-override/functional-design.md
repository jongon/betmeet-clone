...
| **BR-48.9** | El leaderboard del pool calcula el total de cada miembro: para cada match, sumar puntos del override si existe; si no, sumar puntos de la global. Sin doble conteo. **Acotado por Unit 55 (BR-55.1, 2026-06-20)**: solo cuentan los matches con `kickoffAt ≥ PoolMembership.joinedAt` del miembro (los partidos jugados antes de su ingreso al pool no suman), de modo que el total del pool refleje lo acumulado dentro del pool y no el histórico global completo. Ver `construction/unit-55-pool-leaderboard-membership-scoped/functional-design.md`. | FR-REFINE-48.5, FR-REFINE-55.1 |
...
| **BR-48.16** | La query `getPoolMemberPredictions` carga todos los matches (sin filtro `kickoffAt`) y devuelve `{ matches, predictions }`. El componente agrupa por dia via `buildDayGroups(allMatches)`, pagina via `paginateDays(days, timeZone, page)`, y renderiza solo la pagina activa. Las columnas de matches sin predicciones se muestran con celdas vacias. **Acotado por Unit 53 (BR-53.1, 2026-06-20)**: quitar el filtro `kickoffAt` expuso las predicciones futuras de **otros** miembros (regresión de la garantía anti-sesgo de Unit 41/FR-REFINE-41.1). Unit 53 reintroduce el enmascarado server-side de las predicciones de `miembro !== viewer` para partidos no iniciados; el viewer sigue viendo/editando sus propias predicciones futuras (los matches y columnas se siguen cargando). Ver `construction/unit-53-pool-predictions-hide-future/functional-design.md`. | FR-REFINE-48.9, FR-REFINE-53.1 |
