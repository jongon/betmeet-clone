...
- [x] **1. Transform `partitionDaysByToday`** en `src/features/predictions/services/fixture-by-day.ts`
      — recibe `FixtureDayGroup[]` + `today` (`yyyy-mm-dd` UTC), devuelve `{ pastDays, currentDays }`.
...
- [x] **4. `page.tsx`** — calcular `today` UTC, `partitionDaysByToday`, interpolar labels con el
