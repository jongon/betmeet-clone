...
| **BR-47.5** | `PoolSettingsCard` se renderiza para `pool.isOwner` en cualquier tipo de pool (ya no requiere `type === "PRIVATE"`). **(Nota Unit 54, 2026-06-20)**: en el código shipped, esta apertura del gate de render del card **no** se aplicó en Unit 47 (el card siguió condicionado a PRIVATE); **Unit 54 la materializa** al abrir el gate de `page.tsx` a `pool.isOwner` (para que el dueño pueda renombrar pools PUBLIC y PRIVATE). |
| **BR-47.6** | `updatePoolMembersCanInvite` acepta cualquier tipo de pool. El gate owner-only se mantiene. Se elimina el guard `pool.type !== "PRIVATE"`. **(Nota Unit 54, 2026-06-20)**: ⚠️ discrepancia con el código shipped — `update-pool-members-can-invite.ts` **conserva** el guard `if (pool.type !== "PRIVATE")` ("el toggle solo aplica a PRIVATE; los PUBLIC siempre permiten invitar"). El **toggle** `membersCanInvite` sigue solo-PRIVATE; Unit 54 no lo cambia (solo abre el render del card, BR-47.5/BR-54.3). |
