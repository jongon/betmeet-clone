...
| `500` con cuerpo vacío en scopes de proveedor | `revalidateResultViews` (`next/cache`) lanzaba en el Route Handler. | Corregido en dos pasos: (1) Unit 50 — revalidate best-effort + handler devuelve el error en el body (evita el `500`). (2) Unit 52 — **causa raíz**: lanzaba `updateTag` (E872, Server-Action-only). El paso (1) solo silenciaba la excepción, dejando el cache **sin invalidar** → doble refresh en `/matches`. Ahora usa `revalidateTag(tag, "max")` (válido en Route Handlers) y la invalidación sí ocurre. |
| **Doble refresh** en `/matches`/`/rankings`: la primera carga muestra datos viejos, el segundo refresco los actualiza | Caches `unstable_cache` (`revalidate: 300`) que **no se invalidaban** porque la invalidación del cron usaba `updateTag` (lanza `E872` en Route Handlers) → solo se refrescaban por su ventana de 300s con stale-while-revalidate. | Corregido en Unit 52: `updateTag` → `revalidateTag(tag, "max")`. **Regla**: en Next.js 16 los caches `unstable_cache` se invalidan con `revalidateTag(tag, "max")`, **nunca** con `updateTag` (Server-Action-only; lanza desde crons/Route Handlers). Verificación en prod: tras un sync, `/matches` debe mostrar el marcador nuevo al **primer** refresco. |
