...
En la nueva fase eliminatoria, cuando un partido empata en los 120 minutos se define por **tanda de penales**. football-data.org devuelve, para esos partidos, `score.duration = "PENALTY_SHOOTOUT"` con el resultado **del juego** en `regularTime` (+ `extraTime`) y la tanda en `penalties`, pero **el `score.fullTime` es el agregado con la tanda incrustada** — codificación documentada del proveedor (`overtime.html`): `fullTime = regularTime + extraTime + penalties` (p. ej. Alemania 1–1 Paraguay, tanda 3–4, llega como `fullTime` 4–5). El provider `FootballDataProvider` mapeaba `homeScore/awayScore = fullTime`, por lo que el **resultado de la tanda se mostraba como marcador final del partido** (incorrecto). El provider ahora separa: `homeScore/awayScore` = goles del partido (juego corrido, `regularTime` + `extraTime`; `fullTime` cuando no hay tanda) y `homePenaltyScore/awayPenaltyScore` = **la tanda, derivada de `fullTime − (regularTime + extraTime)`**. Se deriva del `fullTime` (campo de marcador siempre mantenido) y **no** del desglose `penalties` porque éste puede venir **corrupto**: para el partido de referencia llegó como empate imposible `4-4` con `winner: null`, violando la invariante (`1-1 + 4-4 = 5-5 ≠ fullTime 4-5`), mientras que `fullTime 4-5` decodifica a la tanda real `3-4`. Se rechaza una tanda derivada empatada (imposible) y se cae al campo `penalties` solo si `fullTime` no la trae. El sync persiste ambos en las columnas existentes `home_penalty_score`/`away_penalty_score` y **deriva `winner_team_id`** (por marcador, o por la tanda si se empató en el juego) respetando el congelado de overrides/regresión terminal — con lo que el bonus de penales en el scoring también funciona en partidos auto-sincronizados. Partido de referencia: **Alemania vs Paraguay** (su dato corrupto se fijó además con override de admin congelado; ver functional-design §1bis y audit).
...
## Épica 76: El error de validación de la predicción en `/matches` ya no bloquea el formulario (Unit 76 — añadida vía refine, 2026-07-07)

### FR-REFINE-76.1 — Un error de validación recuperable en `/matches` deja el formulario editable para corregir inline
En la lista de partidos (`/matches`), al guardar una predicción inválida —caso típico: un **empate en fase de eliminación sin elegir quién avanza por penales** (`validatePredictionInput`, BL-5.2)— la app muestra el mensaje de error (correcto y deseado) pero **mantiene el formulario editable** para corregirlo **ahí mismo** (elegir el ganador en el `PenaltyWinnerSelector` y volver a guardar), **sin recargar la pantalla**. Antes, `prediction-form.tsx` trataba **cualquier** resultado no-`success` de `savePrediction` como un conflicto de bloqueo (`setLockedConflict(true)` + `setCanEdit(false)`), lo que por `isReadOnly = NOT liveCanEdit OR lockedConflict` (Unit 57) volvía el form solo-lectura y ocultaba los controles +/−, el `PenaltyWinnerSelector` y el botón Guardar → la única salida ante un error de validación era recargar. La causa de fondo: `savePrediction` devolvía la misma forma `{ error: string }` para conflictos de elegibilidad reales (kickoff alcanzado) y para errores recuperables (validación / no-miembro / transitorio). El fix añade un discriminante `locked: true` **solo** a las ramas de conflicto de elegibilidad real; el cliente pasa a solo-lectura **solo** ante `locked`, y muestra el resto de errores dejando el form editable. Puramente presentacional/UX: BR-5.5/5.7 intactas (la autoridad sigue 100% server-side); el lock en vivo por kickoff (Unit 57) queda intacto.

### Restricciones / SKIP (Unit 76)
- Alcance **solo `/matches`** (`prediction-form.tsx`). El modal de `/pools` (`pool-predictions-view.tsx`) ya trataba los errores con `toast.error` dejando el modal abierto; el campo `locked` es opcional y retrocompatible, no lo afecta.
- **Prevención proactiva (BR-76.7)**: además de dejar el form editable ante el error, el botón «Guardar» se **deshabilita** (`needsPenaltyWinner = Boolean(showPenaltySelector) && penaltyWinner === null`) mientras el `PenaltyWinnerSelector` esté visible y no se haya elegido ganador, para que la predicción **nunca** llegue a rechazarse por ese motivo. La validación server-side (`validatePredictionInput`) queda como red de seguridad.
