...
| 39. Sync — unique constraint in `providerTeamId` | Units 4, 25, 29, 33 | None | Schema fix; drop `@unique` on `providerTeamId` |
| 40. Admin Sync Select Dark Mode | Units 7, 8 | None | UI-only; contrast fix for sync scope select |
| 41. Pool Predictions View | Units 3, 5, 6 | None | "Predicciones" tab in `/pools/[id]` |
| 42. Matches Local Day Grouping | Units 16, 30, 41 | None | Timezone-aware day grouping in `/matches` |
...
| 50. Sync & Scoring automáticos (Crons) | Units 4, 6, 7, 25, 28, 43, 46 | None | Resuelve FR-06; Supabase pg_cron + pg_net → `POST /api/cron/sync` (guard `x-sync-secret`); orquestación compartida `runScheduledSync`; migración pg_cron defensiva; conserva sync manual del admin |
...
23. After Unit 43: users can enable push from onboarding; notifications dispatch automatically after admin sync.
24. After Unit 44: any pool member can send a directed invite via autocomplete.
25. After Unit 45: pool owner can configure whether non-owner members can send directed invites (toggle in `/pools/[id]` Settings + at pool creation; `FR-REFINE-44.7` superseded; `Pool.membersCanInvite` default `true`).
26. After Unit 50: with `SYNC_TRIGGER_SECRET` + Vault secrets set, `POST /api/cron/sync?scope=…` returns 401 without the header, 200 with it; live scores and points update on the pg_cron schedule with no admin action; `LIVE_STATUS` skips the provider when no match is live/imminent; the admin manual sync still works as fallback.
