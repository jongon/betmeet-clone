...
export function SessionList({ sessions }: { sessions: ReadonlyArray<Session> }) {
  ...
  const filtered = useMemo(() => filterSessions(sessions, name, status), [sessions, name, status]);
  ...
  return (
    <section className="space-y-4">
      {!isServerEmpty ? (
        <FilterBar name={name} onNameChange={setName} status={status} onStatusChange={setStatus} />
      ) : null}
