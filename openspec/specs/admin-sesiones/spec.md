...
- **THEN** la página renderiza las 2 abiertas en la parte superior ordenadas de más nueva a más vieja por `createdAt`, y la cerrada debajo sin importar su fecha

#### Scenario: No hay sesiones
- **WHEN** el admin autenticado navega a `/admin` y el repositorio está vacío
...
El sistema SHALL garantizar que una sesión con `status: "closed"` no pueda transicionar de vuelta a `open`. Si una Server Action (`acceptSession` o `rejectSession`) recibe un `id` de una sesión que ya está `closed`, el sistema SHALL tratarlo como un no-op y SHALL no modificar el repositorio. El botón ✓ y el botón ✗ SHALL renderizarse únicamente en filas con `status: "open"`.
