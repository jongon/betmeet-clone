...
### 6) UI admin en la misma fila
- **Decision:** la edicion de ambos componentes se hace en la misma fila de la lista de cromos, con la misma pantalla en mobile. El preview legible aparece solo al editar.
- **Rationale:** reduce friccion y mantiene paridad desktop/mobile.
- **Alternatives considered:**
...
### 7) Validacion contra faltantes con importacion directa
...
### 9) Rechazo automatico de propuestas con cromo exacto no faltante
...
6. Extender la UI de `/admin/intercambio` para editar ambos componentes en la misma fila, con preview solo al editar.
7. Integrar el rechazo automatico en el envio de propuestas cuando un cromo exacto ya no es faltante.
8. Cubrir con tests: lectura tolerante, persistencia, toggle de limpieza, orden de componentes, validacion de faltantes y rechazo automatico.
9. Despliegue no destructivo; rollback consiste en volver al modelo anterior y dejar de exponer el componente exacto.
...
- Queda por definir el orden visual de los componentes en la fila admin: apilados verticalmente, en columnas, o en un unico bloque expandible al editar.
