...
async function run() {
  ...
  try {
    ...
    const cssMatch = html.match(/href="(\/_next\/static\/(?:css|chunks)\/[^"]+\.css)"/);
    if (!cssMatch) {
      ...
      check(
        ...
        (/oklch\([^)]*0\.349[^)]*251\)/.test(css) || /--primary:\s*#1a3c5e/.test(css)) &&
          (/oklch\([^)]*0\.612[^)]*22\)/.test(css) || /--brand:\s*#e63947/.test(css)) &&
          (/oklch\([^)]*0\.969[^)]*106\)/.test(css) || /--background:\s*#f5f5f0/.test(css)),
      );
      check(
    ...
    check("Página muestra título 'Cromos Mundial 2026'", /Cromos Mundial 2026/.test(html));
