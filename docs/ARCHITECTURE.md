...
- **Redirecciones del proxy**: el middleware es la autoridad del enrutado por sesión. El usuario **no autenticado** que pide una ruta protegida va a `/sign-in?next=…`; el **no confirmado** a `/verify-email`; el que **no completó onboarding** a `/onboarding/profile?next=…`. El usuario **con sesión** que entra a una página de solo-auth (`/sign-in`, `/sign-up`, `/forgot-password`, `/verify-email`) **o a la portada `/`** es redirigido a `/matches` (FR-REFINE-77.1) — salvo una sesión aal1 con **MFA pendiente**, que se deja pasar para completar el reto TOTP. `/` permanece **pública** para el visitante anónimo (ve el landing); el landing de `src/app/page.tsx` conserva su render consciente de sesión como degradación elegante.
